...
Separates too-small filtering from low-support filtering using measured
connected-component and 7-plane fixed-XY support, with explicit edge handling
against the original (non-eroded) tissue mask. Provisional candidates are never
called cells and nothing is auto-promoted. Raw TIFFs, candidate coordinates,
detection thresholds, injection masks, and existing run outputs are read only;
outputs go into the supplied isolated ``--out-dir``.

Modes:
  --mode report  Diagnostics + threshold sweep only; no status changed.
  --mode apply   Applies ONLY explicitly supplied thresholds; refuses to run
...
from mouse_brain_pipeline.strict_refinement import (
    MODE_APPLY,
    MODE_REPORT,
    StrictThresholds,
...
def _prepare_output_root(out_dir: Path, run_dir: Path, channel: str) -> Path:
    ...
    marker_path = out_dir / "strict_refinement_run.json"
    expected_run = str(run_dir.resolve())
    if out_dir.exists():
        entries = list(out_dir.iterdir())
        if entries and not marker_path.is_file():
            raise FileExistsError(
                f"--out-dir is not an isolated strict-refinement folder: {out_dir}"
            )
    ...
    marker_path.write_text(
        json.dumps(
            {"run_dir": expected_run, "analysis": "strict candidate refinement"},
            indent=2,
        ),
        encoding="utf-8",
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Stricter candidate size/support/edge refinement (report/apply)."
    )
    ...
    parser.add_argument("--min-support-voxels", type=int, default=None)
    parser.add_argument("--max-edge-distance-penalty-um", type=float, default=None)
    parser.add_argument("--rescue-edge-candidates", action="store_true")
    return parser
...
def run_cli(args: argparse.Namespace) -> dict:
    ...
    thresholds = StrictThresholds(
        ...
        min_support_voxels=args.min_support_voxels,
        max_edge_distance_penalty_um=args.max_edge_distance_penalty_um,
        rescue_edge_candidates=bool(args.rescue_edge_candidates),
    )
    if args.mode == MODE_APPLY and not thresholds.has_any_size_or_support_threshold():
        raise SystemExit(
    ...
    if (
        ...
        raise SystemExit(
            "--rescue-edge-candidates requires --max-edge-distance-penalty-um in apply mode"
        )
    ...
    result = refine_candidates_strict(
        rows,
        tissue_mask,
    ...
    outputs = write_strict_outputs(channel_dir, result, make_plots=True)
    result.summary["outputs"] = outputs
    (channel_dir / "strict_refinement_summary.json").write_text(
        json.dumps(result.summary, indent=2), encoding="utf-8"
    )

    print(f"{args.channel}/{args.mode}: {len(rows)} candidates")
    print(f"Wrote strict refinement outputs to {channel_dir}")
    return outputs
